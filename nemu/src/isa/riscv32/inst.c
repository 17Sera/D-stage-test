...
// 	INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm );//no imm << 12 --- or crc32 mersenne fail
// 	INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));
//   INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));//zext.b
//   INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
// 	//INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s -> pc + 4; s -> dnpc += imm -4);
//   //INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s -> dnpc = s -> pc + imm;if(rd == 1){display_call_func(s->pc, s->dnpc);});
//   //INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s -> pc + 4; s -> dnpc += imm -4;if(rd == 1){display_call_func(s->pc, s->dnpc);}); // jal指令
//   INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm;IFDEF(CONFIG_FTRACE, if(rd == 1){display_call_func(s->pc, s->dnpc); }));

// 	INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
// 	INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2);
// 	INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
//   INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = ((src1 + imm)) & ~1; IFDEF(CONFIG_FTRACE, if(rd == 1){display_call_func(s->pc, s->dnpc);}else if(rd == 0 && src1 == R(1)){display_ret_func(s->pc);}));
// 	//INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s -> pc + 4; s -> dnpc = (src1 + imm) & ~1);//ret
//   //INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr    , I, R(rd) = s -> pc + 4; s -> dnpc = (src1 + imm) & ~1;if(rd == 1){display_call_func(s->pc, s->dnpc);}else if(rd == 0 && src1 == R(1)){display_ret_func(s->pc);}); 
...
// 	INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl , R, R(rd) = (uint32_t)src1 >> (uint32_t)BITS(src2, 4, 0));
// 	INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd)= src1 >> (uint32_t)imm);
...
// 	INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (uint32_t)src1 / src2);
...
/***********************************************RV32I Base Instruction Set***********************************************/
  ...
  //INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & ~1; R(rd) = s->pc + 4; IFDEF(CONFIG_FTRACE, if(rd == 1){display_call_func(s->pc, s->dnpc);}else if(rd == 0 && src1 == R(1)){display_ret_func(s->pc););  
  // INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = ((src1 + imm)) & ~1; IFDEF(CONFIG_FTRACE, if(rd == 1){display_call_func(s->pc, s->dnpc);}else if(rd == 0 && src1 == R(1)){display_ret_func(s->pc);}));
  ...
  //INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + (imm << 1); R(rd) = s->pc + 4; IFDEF(CONFIG_FTRACE, if(rd == 1){display_call_func(s->pc, s->dnpc);); 
  // INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm;IFDEF(CONFIG_FTRACE, if(rd == 1){display_call_func(s->pc, s->dnpc); }));
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int64_t num1 = (int)src1, num2 = (int)src2; R(rd) = (int)(num1 * num2 >> 32));
  //INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, uint64_t num3 = (int)src1, num4 = (int)src2; R(rd) = (uint32_t)(num3 * num4 >> 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, uint64_t num3 = src1, num4 = src2; R(rd) = (uint32_t)(num3 * num4 >> 32));
