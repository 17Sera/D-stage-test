...
module idu(
                ...
                case (func3)
                    `INST_E_M: begin
                        if(idu_csr_rid == `INST_MRET)
                            ...
                            TRAP(`HIT_TRAP, `Unit_IDU1);
                        else
                            idu_is_mret = `TRUE;//TRAP(`ABORT, `Unit_IDU2); 
                    end
                    `INST_CSRRW: begin idu_csr_type = `CSR_RW; idu_csr_ren = `Enable; end
                    `INST_CSRRS: begin idu_csr_type = `CSR_RS; idu_csr_ren = `Enable; end
                    default: begin idu_csr_type = `CSR_RS; idu_csr_ren = `Enable; end//TRAP(`ABORT, `Unit_IDU3);  
                endcase
            end
            `TYPE_R,`TYPE_I,`TYPE_I_LOAD,`TYPE_I_JALR,`TYPE_STORE,`TYPE_B,`TYPE_U_LUI,`TYPE_U_AUIPC,`TYPE_JAL:
                    idu_csr_type = `CSR_Nop;
            default:  idu_csr_type = `CSR_Nop; //TRAP(`ABORT, `Unit_IDU4);  
        endcase
    end
    ...
    always @(*) begin
        ...
        case (opcode)
            ...
            default: if(opcode != `TYPE_SYS)  idu_alu_type = `ALU_ADD; //TRAP(`ABORT, `Unit_IDU9); 
        endcase
    end
    // to Register File
    assign idu_rs_id1 = (idu_is_ecall == `TRUE) ? `MCASUSE_GPR :  // ecall: src1 = value of a17
